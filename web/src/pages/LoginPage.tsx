...
export function LoginPage() {
  ...
  const reset = Boolean(
    (location.state as { reset?: boolean } | null)?.reset
  );
  ...
  return (
    <AuthCard
      ...
      footer={
        <>
          New to Chugly?{" "}
          <Link to="/register" className="font-bold text-moss hover:text-ink">
            Create an account
          </Link>
        </>
      }
    >
      <form className="space-y-4" onSubmit={handleSubmit} noValidate>
        ...
        <FormField
          ...
          hint={
            <Link to="/forgot-password" className="text-moss hover:text-ink">
              Forgot it?
            </Link>
          }
        >
